...
from river.models import State,TransitionMeta,TransitionApprovalMeta, Workflow,Transition
...
class GetUserAllData(APIView):
    ...
    def get(self,request,format = None):
        ...
        if user.filter(name='Teacher').exists():
            data = StudentBase.objects.values('user')# "objects.values()" :give the specified values from the model
...
class ApproveInstanceApi(APIView):
    ...
    def post(self,request,format = None,pk=id):
        # print(request.user) # showsthe user whose token is given.
        try:
            
            
            studentsData = StudentBase.objects.get(pk = pk)
            print(studentsData.status)
            
            data =studentsData.river.status.get_available_approvals(
                        as_user=request.user,).values()
            print(data)
            print(dir(StudentBase.river.status))
            studentsData.river.status.approve(as_user=request.user)
